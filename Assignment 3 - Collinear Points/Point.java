...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        double slope;
        if ((that.y == y) && (that.x == x)) slope = Double.NEGATIVE_INFINITY;
        else if (that.y - y == 0) slope = +0.0;
        else if (that.x - x == 0) slope = Double.POSITIVE_INFINITY;
    ...
    private class BySlope implements Comparator<Point>
    {
        public int compare(Point q1, Point q2) 
        {
            ...
            double pq = p.slopeTo(q1);
            double pr = p.slopeTo(q2);
